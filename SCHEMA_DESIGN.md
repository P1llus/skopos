...
| 4     | 2026-05-15 | `Value.FromPagination` and `Value.FromProgress` deleted. Templates / fixtures / tests use `{ref: cursor.<name>}` for every pagination + progress signal — `cursor.token`, `cursor.page`, `cursor.offset`, `cursor.offset_end`, `cursor.scroll_id`, `cursor.next_link`, `cursor.next_url`, `cursor.<cursor_var>` (graphql_relay), `cursor.last_timestamp`, `cursor.window_start`, `cursor.window_end`. Runtime no longer keeps `scope.fromPagination` / `scope.fromProgress` maps; `pagination.seed` / `progress.seed` write directly into `scope.cursor` and the http auto-injector reads `scope.cursor[role]` for `send_as` lowering. `schema.cursorSchema` registers `cursor.offset_end` only when `pagination.offset.batch_size` is set, and `cursor.last_timestamp` for `async_job` only when `on_complete.cursor_update.kind` is `use_now` or `latest_event_timestamp` — preserving the old role-membership rules through the cursor-namespace validator. Codecs reject the deleted `from_pagination:` / `from_progress:` discriminator keys at parse time with migration hints (mapping each old role to its new `cursor.<name>`). Templates that used to emit `since=""` on the bootstrap drain (`async_poll`, `async_poll_latest_ts`) carry an explicit `default: ""` on the ref to preserve wire shape. New `TestSliceFourFromPaginationProgressDeleted` covers parse-time rejection (YAML + JSON), positive cursor-namespace validation under each strategy, `cursor.offset_end` requiring `batch_size`, and the `async_job` cursor-update kind matrix. |
| 5     | 2026-05-15 | `send_as` auto-injection deleted end-to-end. `schema.CursorTokenPagination` carries only `TokenAt Path`; `schema.ScrollIDPagination` carries `ScrollIDAt Path` plus the optional `CompleteWhen *Predicate`. Custom YAML/JSON unmarshalers reject leftover `send_as:` keys at parse time with hints pointing at the new explicit-wire form. `client/pagination.go` lost the `paginationAutoInjector` interface, the `autoInjectSlot` type, every `autoInjectSlot()` driver method, and the `parseSendAs` helper. `client/http.go::executeRequest` no longer takes an `inject` parameter (signature trimmed; no implicit query/header lowering branches); the `queryDeclared` / `headerDeclared` helpers are gone. `client/runner.go::runIteration` no longer threads `reqInject` through `executeRequest`. `templates/cursor_token.yml`, `templates/oauth2_client_credentials.yml` now wire `query.cursor: {ref: cursor.token, default: ""}` so the bootstrap iteration still sends `cursor=` (empty); `templates/scroll_id.yml` wires `query.scroll: {ref: cursor.scroll_id}` (no default — the bootstrap iteration omits the slot so the server opens a fresh scroll). Goldens for `cmd/skopos/testdata/schema/{cursor_token_placeholder,predicate_composition,token_cache}.txt` and the corresponding `schema/testdata/*.yml` fixtures regenerated. The legacy `cursor_token_implicit` fixture + golden deleted (the implicit form ceased to exist). `cmd/skopos/testdata/errors/send_as_bad_format.txt` repurposed to assert the new "send_as was removed" parse-time hint. New `TestSliceFiveSendAsDeleted` in `schema/fixtures_test.go` covers parse-time rejection (YAML + JSON, struct-level + full-doc) for both cursor_token and scroll_id, plus a positive case that the explicit `{ref: cursor.token, default: ""}` form validates cleanly. End-to-end runtime tests `TestEndToEnd_CursorToken_Explicit` / `TestEndToEnd_ScrollID_Explicit_Header` replace the deleted implicit / explicit-wins triad. End-to-end smoke against the testserver confirms `cursor_token`, `scroll_id`, and `oauth2_client_credentials` still drain correctly. |
| 6     | 2026-05-15 | Predicate right-hand-side renamed: YAML/JSON tag `equal:` → `value:`, Go field `PredicateEq.Equal` → `PredicateEq.Value`. `PredicateEq` gains custom YAML/JSON unmarshalers (mirroring the slice-3 / -4 / -5 shadow-struct pattern) that reject the legacy `equal:` key at parse time with `"predicate eq.equal was renamed to eq.value in slice 6 (shared across eq / gt / lt / gte / lte); use value: instead"`. The Go type name `PredicateEq` is unchanged — slice plan calls that a separate cosmetic rename. Every validator path-string flips from `*.eq.equal` to `*.eq.value` (and the same for gt / lt / gte / lte), every `pe.Equal` reference in `client/predicate.go`, `client/value.go::predicateContainsSecret`, and the test struct literals in `client/{predicate,progress,auth,value,pagination}_test.go` flip to `pe.Value` / `Value:`. Templates (`scroll_id`, `etag_conditional`, `etag_conditional_middle`, `multi_mode_auth`, `async_poll`, `async_poll_latest_ts`, `async_poll_stateless`) and every `schema/testdata/*.yml` fixture with a predicate verb regenerate with `value:`. Goldens under `cmd/skopos/testdata/schema/` + the five `cursor_update_*.txt` error goldens flip in lockstep. New `TestSliceSixEqualRenamedToValue` in `schema/fixtures_test.go` covers parse-time rejection (YAML + JSON, struct-level + full-doc, every comparison verb) plus a positive case that round-trips `value: 1` through `PredicateEq.Value`. `docs/schema-reference.md` regenerated. End-to-end smoke against the testserver confirms `scroll_id`, `async_poll`, and `multi_mode_auth` still drain. |
| 7     | 2026-05-15 | Cache expiry slots moved out of `cursor.__*_expires_at` into `state.<store_in>_expires_at`. `client/oauth2.go::oauth2ExpiryKey` and `client/requestcache.go::stepCacheExpiryKey` now return the unprefixed `<store_in>_expires_at` and read/write through `s.state` (cachedOAuth2Token, storeOAuth2Token, clearOAuth2Cache, cachedStepValue, storeStepValue, invalidateStepCaches). The runner's `on_status: invalidate_cache` arm clears both the token and the expiry slot from state for every reachable cache. `schema/validate.go::preregisterStateAndCursor` auto-registers `<store_in>_expires_at` as a runtime `string` state field alongside the cache `store_in` (for both `auth.oauth2.<grant>.cache` and `requests[].cache`); the validator's cache-store_in collision check additionally rejects an author-declared `state.fields.<store_in>_expires_at` whenever a cache block claims the slot, with a hint naming "paired expiry slot". `Snapshot.Cursor` carries only author-facing pagination / progress / async_job state — the underscore-prefixed framework-internal keys are gone. `client/{auth,requestcache}_test.go` flipped every `snap.Cursor["__oauth2_token_expires_at"]` / `snap.Cursor["__step_session_token_expires_at"]` poke to the new state-rooted slot, and the test helpers (`oauth2Doc`, `sessionLoginCachedDoc`) auto-register `<store_in>_expires_at` as runtime/string to mirror the IR validator. `templates/session_login_cached.yml` and `cmd/skopos/testdata/session_login_cached.txt` reference the new slot in their comments / grep targets. `docs/{stores,runtime,schema,api-methods}.md` updated to describe the new slot location. New `TestSliceSevenExpirySlotsMoved` covers OAuth2 + request-cache auto-registration of the expires_at slot, the validator's collision rejection for both cache flavours, and the rejection of a leftover `cursor.__oauth2_token_expires_at` ref as an unknown cursor field. End-to-end smoke against the testserver confirms `oauth2_client_credentials`, `oauth2_relay`, and `session_login_cached` write the expiry under `state.*` not `cursor.*`. |
| 8     | 2026-05-16 | Doc sweep + `schema-reference.md` regen. Every legacy `body.<path>` mention flipped to `response.body.<path>` (or `steps.<id>.body.<path>`); the legacy "namespace-shadow warning" section in `docs/schema.md` deleted; `from_pagination` / `from_progress` Value variants removed from the discriminated-forms table and replaced with plain `{ref: cursor.<name>}` wiring across `docs/schema.md`, `docs/api-methods.md`, `docs/runtime.md`, `docs/stores.md`, and `README.md`. `ExtractVar` table flipped to the single `from:` field (slice 3); `send_as` rows / sections deleted from `pagination.cursor_token` / `pagination.scroll_id` and replaced with a "Wiring pagination signals" table that names the `cursor.<name>` field per strategy (slice 5). Every Predicate example flipped to `value:` (slice 6). Cursor-namespace table tightened to list only the names each strategy actually populates (no parallel role names). Doc YAML examples updated in lockstep (`events_at: response.body.data.events`, `expiry_field: response.body.expires_in`, `extract.from: response.header.Set-Cookie`, async-job `extract: {<name>: {from: response.body.<path>}}`). `templates/async_poll_latest_ts.yml` comment de-referenced `from_progress` wording. README quickstart trace line refreshed to use `<ref cursor.last_timestamp>` (post-slice-4 redact output). `docs/schema-reference.md` round-trips through `gen-schema-doc -check` cleanly (slice-7 commit already covered the generator output — no regeneration delta needed). Manual copy-paste validation: pagination + cursor wiring, multi_mode auth + predicate `value:`, multi-step extract via `from: response.header.<name>`, and `progress.time_window` with `cursor.window_*` refs all round-trip through `skopos validate -i <file>`. Gate: `go build`, `go test ./...`, `go vet ./...`, `go run ./tools/gen-schema-doc -check`, and `go test -tags integration ./cmd/skopos/...` all green. |
