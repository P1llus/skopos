...
// ---- bearer_simple end-to-end ----

// TestEndToEnd_BearerSimple drives the bearer_simple template against an
// httptest server. It asserts the request carries Authorization: Bearer
// <token>, the since= window equals the initial lookback, the events come
// through, and cursor.last_timestamp advances to the max event time.
func TestEndToEnd_BearerSimple(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if got := r.Header.Get("Authorization"); got != "Bearer test-token" {
			t.Errorf("Authorization = %q, want Bearer test-token", got)
		}
		if got := r.URL.Query().Get("since"); got == "" {
			t.Errorf("missing since= query param")
		}
		writeJSON(w, http.StatusOK, map[string]any{
			"events": []map[string]any{
				{"id": "e1", "timestamp": "2026-05-12T08:00:00Z"},
				{"id": "e2", "timestamp": "2026-05-12T09:00:00Z"},
			},
		})
	}))
	defer server.Close()
	...
	if sink.flushN != 1 {
		t.Errorf("Flush called %d times, want 1", sink.flushN)
	}
}

// ---- cursor_token end-to-end ----

// TestEndToEnd_CursorToken drives the cursor_token template across three
// pages and asserts (a) the second/third page carries the previous
// next_cursor as cursor=, (b) the loop terminates when next_cursor is
// empty, and (c) cursor.token is cleared in the final Snapshot.
func TestEndToEnd_CursorToken(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case 0:
			if cur != "" {
				t.Errorf("page 0 cursor = %q, want empty", cur)
			}
		case 1:
			if cur != "tok-2" {
...
// ---- page_number end-to-end ----

// TestEndToEnd_PageNumber drives page_number across two pages with
// meta.has_next as the termination signal.
func TestEndToEnd_PageNumber(t *testing.T) {
	var pageHits atomic.Int32
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		hit := int(pageHits.Add(1))
		page := r.URL.Query().Get("page")
		switch hit {
		case 1:
			if page != "1" {
				t.Errorf("first request page = %q, want 1", page)
			}
			writeJSON(w, http.StatusOK, map[string]any{
				"data": []map[string]any{{"id": "a", "created_at": "2026-05-12T08:00:00Z"}},
				"meta": map[string]any{"has_next": true},
			})
		case 2:
			if page != "2" {
				t.Errorf("second request page = %q, want 2", page)
			}
			writeJSON(w, http.StatusOK, map[string]any{
				"data": []map[string]any{{"id": "b", "created_at": "2026-05-12T08:05:00Z"}},
				"meta": map[string]any{"has_next": false},
			})
		default:
			t.Errorf("unexpected extra page request %d", hit)
		}
	}))
	defer server.Close()
	...
	sink := &captureSink{}
	r := &Runner{
		Doc:    pageNumberDoc(server.URL, "test-token"),
		Sink:   sink,
		Now:    fixedNow(),
		Client: server.Client(),
	}
...
// ---- async_poll end-to-end ----

// TestEndToEnd_AsyncPoll drives the submit/poll/fetch phase machine.
//
// Two cases are interleaved on the same Runner instance:
//
//  1. Poll returns "pending" first → drain stops after poll (caller waits
//     for the next --interval tick). cursor.phase stays at "poll",
//     cursor.export_id is captured.
//  2. Next drain polls again, server returns "complete" → drain proceeds
//     into fetch in the SAME iteration, emits events, resets phase to
//     "submit", and writes cursor.last_timestamp from on_complete.use_now.
//
// This pins both the "poll-pending → wait" semantics and the
// "poll-complete drives into fetch immediately" semantics.
func TestEndToEnd_AsyncPoll(t *testing.T) {
	var submitCount, pollCount, fetchCount atomic.Int32
	var pollPending atomic.Bool
	pollPending.Store(true) // first poll returns pending.
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		...
		case strings.HasSuffix(r.URL.Path, "/data"):
			...
			writeJSON(w, http.StatusOK, map[string]any{
				"items": []map[string]any{
					{"id": "evt-1", "ts": "2026-05-12T09:00:00Z"},
				},
			})
	...
	// Drain 1: submit (1) then poll (pending) → stop. No fetch yet.
	if err := r.Drain(context.Background()); err != nil {
		t.Fatalf("Drain 1: %v", err)
	}
	if submitCount.Load() != 1 || pollCount.Load() != 1 || fetchCount.Load() != 0 {
		t.Fatalf("after drain 1: submit=%d poll=%d fetch=%d (want 1,1,0)",
			submitCount.Load(), pollCount.Load(), fetchCount.Load())
	}
	snap1, _ := store.Load()
	if got := snap1.Cursor["phase"]; got != "poll" {
		t.Fatalf("after drain 1 phase = %v, want poll", got)
	}
	if got := snap1.Cursor["export_id"]; got != "exp-1" {
		t.Fatalf("after drain 1 export_id = %v, want exp-1", got)
	}
	if len(sink.events) != 0 {
		t.Fatalf("after drain 1 emitted %d events, want 0", len(sink.events))
	}

	// Drain 2: poll (complete) → fetch happens in the same iteration.
	if err := r.Drain(context.Background()); err != nil {
		t.Fatalf("Drain 2: %v", err)
	}
	if pollCount.Load() != 2 || fetchCount.Load() != 1 {
		t.Fatalf("after drain 2: poll=%d fetch=%d (want 2,1)", pollCount.Load(), fetchCount.Load())
	}
	if len(sink.events) != 1 {
		t.Fatalf("after drain 2 emitted %d events, want 1", len(sink.events))
	}
	snap2, _ := store.Load()
	if got := snap2.Cursor["phase"]; got != "submit" {
		t.Fatalf("after drain 2 phase = %v, want submit (reset)", got)
	}
	if got := snap2.Cursor["last_timestamp"]; got != "2026-05-12T12:00:00Z" {
		t.Fatalf("on_complete.use_now wrote %v, want fixedNow", got)
	}
	if _, ok := snap2.Cursor["export_id"]; ok {
		t.Errorf("export_id should be cleared after fetch resets the phase")
	}
}
